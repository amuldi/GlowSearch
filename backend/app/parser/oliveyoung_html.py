...
def _parse_product_node(node: Tag, *, base_url: str) -> ProductSourceRecord | None:
    brand = _first_text(node, BRAND_SELECTORS)
    name = _first_text(node, NAME_SELECTORS)

    price = _node_price(node)
    image = _first_image(node, base_url)
    source_url = _first_product_url(node, base_url)
    goods_no = _extract_goods_no(source_url or "") or _extract_goods_no(str(node))
...
def _record_from_mapping(item: dict, *, base_url: str) -> ProductSourceRecord | None:
    name = _pick(item, "name", "productName", "goodsNm", "goodsName", "prdNm")
    brand_raw = _pick(item, "brand", "brandName", "brandNm", "maker")
    if isinstance(brand_raw, dict):
        brand = _pick(brand_raw, "name", "brandName")
    else:
        brand = brand_raw

    image_raw = _pick(item, "image", "imageUrl", "imgUrl", "thumbnail", "goodsImg")
    if isinstance(image_raw, list):
        image_raw = next((value for value in image_raw if isinstance(value, str)), None)

    offers = item.get("offers") if isinstance(item.get("offers"), dict) else {}
    price = (
        parse_krw_price(_pick(item, "regularPrice", "orgPrice", "listPrice", "price"))
        or parse_krw_price(_pick(offers, "price", "highPrice", "lowPrice"))
    )
    source_url = _pick(item, "url", "link", "goodsUrl")
    source_url = urljoin(base_url, source_url) if isinstance(source_url, str) else None
    goods_no = _pick(item, "goodsNo", "goodsNumber", "productId", "id")
    if not goods_no:
...
def _looks_like_product(item: dict) -> bool:
    ...
    has_name = bool(keys & {"name", "productName", "goodsNm", "goodsName", "prdNm"})
    has_product_id = bool(keys & {"goodsNo", "goodsNumber", "productId", "id"})
    has_image = bool(keys & {"image", "imageUrl", "imgUrl", "thumbnail", "goodsImg"})
