...
class SlowVerifiedCachePeriperaCollector:
    ...
    async def search(self, keyword: str, limit: int) -> list[ProductSourceRecord]:
        ...
        return _peripera_verified_records()


class ImmediateVerifiedCachePeriperaCollector:
    name = "oliveyoung:verified-cache"
...
def _peripera_verified_records() -> list[ProductSourceRecord]:
    return [
        ProductSourceRecord(
...
@pytest.mark.asyncio
async def test_search_service_processes_verified_cache_before_live_result_when_both_complete(tmp_path) -> None:
    registry_path = tmp_path / "brand_registry.json"
    registry_path.write_text(
        '{"entries":[{"official_en":"peripera","aliases":["페리페라","peripera"],"sources":[]}]}',
        encoding="utf-8",
    ...
    service = SearchService(
        collectors=[
            FastOliveYoungPublicApiPeriperaCollector(),
            ImmediateVerifiedCachePeriperaCollector(),
        ],
        normalizer=ProductNormalizer(
            BrandResolver(registry_path),
            base_url="https://www.oliveyoung.co.kr",
    ...
    assert response.count == 1
    result = response.results[0]
    assert result.canonical_product_id == "verified:peripera-speedy-skinny-brow"
    assert result.product_name_display_en == "Speedy Skinny Brow"
    assert [offer.source for offer in result.offers] == ["oliveyoung", "official"]
